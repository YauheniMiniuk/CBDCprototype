...
func (s *Erc20Contract) Name(ctx contractapi.TransactionContextInterface) (string, error) {
...
func (s *Erc20Contract) Symbol(ctx contractapi.TransactionContextInterface) (string, error) {
...
func (s *Erc20Contract) Initialize(ctx contractapi.TransactionContextInterface, name string, symbol string, decimals string) (bool, error) {
...
func (s *Erc20Contract) GetChannelID(ctx contractapi.TransactionContextInterface) string {
	id := ctx.GetStub().GetChannelID()
	return id

}

func (s *Erc20Contract) GetHistory(ctx contractapi.TransactionContextInterface, name string) (string, error) {
	historyIter, err := ctx.GetStub().GetHistoryForKey(name)
	if err != nil {
	...
	for historyIter.HasNext() {
		...
		if err != nil {
			return "0", fmt.Errorf("Error in getting History by Key in Iteration", name)
		}
		result += string(modification.Value)
	}
	return result, nil
}

// GetHistoryForKey returns all transactions for a given key (token)
func (s *Erc20Contract) GetHistoryForKey(ctx contractapi.TransactionContextInterface, key string) ([]QueryHistory, error) {
